            ...
            <a href="{{route('chat.private',$user->id)}}" class="d-flex justify-content-between">
                ...
                <div class="pt-1">
                    ...
                    @php
                        ...
                        $user_sent =$allMsg->where('room_name',$channel)->sortByDesc('sent_at')->first()->user_id;
                    @endphp
                    <p id ="textBox-{{$channel}}" class="small text-muted">
                      @if ($user_sent == Auth::guard('admin')->user()->id)
                      You: {{$allMsg->where('room_name',$channel)->sortByDesc('sent_at')->first()->message_text}}</p>
                      @else
                ...
                <div class="pt-1">
                  ...
                  <p id="" class="small  text-success mb-1 notification-{{$user->id}}">online</p>
                  @else
                  <p id="" class="small text-muted  mb-1 notification-{{$user->id}}">offline</p>
                  @endif
                </div>
            </a>
            </li>
